...
namespace SplitEngine::Tools
{
	ImageSlicer::SliceData ImageSlicer::Slice(const std::string& filePath, ImageSlicer::SliceOptions sliceOptions)
	{
		...
		const IO::Image image = IO::ImageLoader::Load(filePath);
		...
		const uint32_t xCount = image.Width / width;
		const uint32_t yCount = image.Height / height;

		if (xCount * yCount < sliceOptions.NumImages)
		{
			ErrorHandler::ThrowRuntimeError(std::format("slicing image {0} failed! {1} image cannot fit with given slicing parameters ", filePath, sliceOptions.NumImages));
		}
		...
		for (uint32_t y = 0; y < yCount; ++y)
		{
			for (uint32_t x = 0; x < xCount; ++x)
			{
				...
				const uint32_t index = (y * xCount) + x;

				const size_t gridOffsetX = x * width * 4;
				const size_t gridOffsetY = y * height * image.Width * 4;

				for (uint32_t line = 0; line < height; line++)
				{
					const size_t lineOffset      = (line * image.Width * 4);
					const size_t sliceLineOffset = (line * width * 4);
