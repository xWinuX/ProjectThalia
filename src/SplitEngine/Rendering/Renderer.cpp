...
namespace SplitEngine::Rendering
{
	...
	void Renderer::BeginRender()
	{
		...
		const vk::ResultValue<uint32_t> imageIndexResult = device->GetVkDevice().acquireNextImageKHR(device->GetSwapchain().GetVkSwapchain(),
		                                                                                             UINT64_MAX,
	...
	void Renderer::EndRender()
	{
		...
		const vk::SubmitInfo   submitInfo   = vk::SubmitInfo(_vulkanContext.GetImageAvailableSemaphore(),
		                                                     waitStages,
		...
		const vk::PresentInfoKHR presentInfo = vk::PresentInfoKHR(_vulkanContext.GetRenderFinishedSemaphore(),
		                                                          device->GetSwapchain().GetVkSwapchain(),
		                                                          _latestImageIndexResult,
		                                                          nullptr);

		const vk::Result presentResult = device->GetPresentQueue().presentKHR(presentInfo);
