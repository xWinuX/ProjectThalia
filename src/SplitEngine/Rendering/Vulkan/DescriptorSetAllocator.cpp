...
{
	DescriptorSetAllocator::DescriptorSetAllocator(Device* device, CreateInfo& descriptorSetInfo, uint32_t maxSetsPerPool) : DeviceObject(device),
	...
	DescriptorSetAllocator::Allocation DescriptorSetAllocator::AllocateDescriptorSet()
	{
		...
		vk::DescriptorSetAllocateInfo  descriptorSetAllocateInfo = vk::DescriptorSetAllocateInfo(validDescriptorPoolInstance->DescriptorPool, Device::MAX_FRAMES_IN_FLIGHT,
		...
		for (int i = 0; i < _writeDescriptorSets.size(); ++i)
		{
			...
			switch (lastWriteDescriptor.descriptorType)
			{
				...
				case vk::DescriptorType::eStorageBuffer:
				{
					...
					if (bufferCreateInfo.DeviceLocal)
					{
					...
					descriptorSetAllocation.ShaderBuffers.push_back(std::move(Buffer(GetDevice(), usage, vk::SharingMode::eExclusive, allocationCreateInfo, numSubBuffers,
					...
					for (int j = 0; j < Device::MAX_FRAMES_IN_FLIGHT; ++j)
					{
						...
						if (!bufferCreateInfo.SingleInstance)
						{
							offset += buffer.GetSizeInBytes(0);
						}
					}
				...
				case vk::DescriptorType::eCombinedImageSampler:
				{
					...
					{
						descriptorSetAllocation.ImageInfos.back().emplace_back(*GetDevice()->GetDefaultSampler(), GetDevice()->GetDefaultImage().GetView(),
	...
	{
		const vk::DescriptorPoolCreateInfo descriptorPoolCreateInfo = vk::DescriptorPoolCreateInfo(vk::DescriptorPoolCreateFlagBits::eFreeDescriptorSet, _maxSetsPerPool,
		...
		DescriptorPoolInstance descriptorPoolInstance = {
			GetDevice()->GetVkDevice().createDescriptorPool(descriptorPoolCreateInfo), std::vector<vk::DescriptorSet>(_maxSetsPerPool, VK_NULL_HANDLE), std::move(availableStack)
		};
	...
	void DescriptorSetAllocator::DeallocateDescriptorSet(DescriptorSetAllocator::Allocation& descriptorSetAllocation)
	{
		if (descriptorSetAllocation.DescriptorSets.IsValid())
		{
			...
			for (auto& buffer: descriptorSetAllocation.ShaderBuffers)
			{
				buffer.Destroy();
			}
	...
	const vk::DescriptorSetLayout& DescriptorSetAllocator::GetDescriptorSetLayout() const
	{
