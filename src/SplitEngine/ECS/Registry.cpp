...
namespace SplitEngine::ECS
{
	...
	Registry::~Registry()
	{
		...
		for (const std::vector<SystemEntry>& systems : _systems)
		{
			for (const SystemEntry& systemEntry : systems) { delete systemEntry.System; }
		}
	...
	void Registry::RemoveQueuedSystems()
	{
		for (const auto& systemsToRemoveID : _systemsToRemove)
		{
			...
			uint64_t                  stage   = static_cast<uint64_t>(systemLookupEntry.Stage);
